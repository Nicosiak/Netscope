...
def stats_from_rtt_history(history: Sequence[Optional[float]]) -> Dict[str, Optional[float]]:
    ...
    # ── Inter-arrival jitter (MAD of consecutive non-loss pairs) ───
    # Reset across packet-loss events — do not span a gap.
    diffs: List[float] = []
    ...
    for v in history:
        if v is not None:
            ...
            prev = None  # gap: next valid sample starts a fresh pair
